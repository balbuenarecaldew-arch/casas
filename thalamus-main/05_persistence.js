...
// ── Cache helpers ──
function saveCache(){
  try{localStorage.setItem('th_cache',JSON.stringify({obras,gastos,certificados,actividades,retiros,gestorPagos,ayudaSocialPagos,contratistaPagos,cfg,_ts:Date.now()}));}catch(e){}
}
function loadCache(){
...
window.initApp = async function(){
  ...
  if(cache&&cache.obras&&Object.keys(cache.obras).length){
    obras=cache.obras; gastos=cache.gastos||{}; certificados=cache.certificados||{}; actividades=cache.actividades||[]; retiros=cache.retiros||{fernando:[],wuilian:[]}; gestorPagos=cache.gestorPagos||[]; ayudaSocialPagos=cache.ayudaSocialPagos||[]; contratistaPagos=cache.contratistaPagos||[];
  ...
  try{
    ...
    const [gastosArr, certsArr, actsDoc, rd, gp, asp, ctp] = await Promise.all([
