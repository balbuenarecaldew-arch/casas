...
function renderObrasGrid(){
  ...
  g.innerHTML=list.map((o,idx)=>{
    ...
    const actCount=window.countActividadesObra?window.countActividadesObra(o.id):0;
    return`<div class="obra-card ${o.id===cur?'act':''}" onclick="selectObra('${o.id}')">
      ...
      <div class="oc-stats">
        ...
        <span class="oc-stat">Actividades: ${actCount}</span>
      </div>
      <div style="margin-top:.46rem;display:flex;gap:.3rem;flex-wrap:wrap">
        ...
        <button class="btn btn-ghost btn-xs" title="Libro diario" onclick="event.stopPropagation();openActividadWindow('${o.id}')">📎 Diario</button>
...
window.delObra=function(id){
  ...
  requireAuth('⚠️ Eliminar obra "'+o.nombre+'".\nSe borrarán todos sus datos.',async()=>{
    ...
    if(actividades.some(a=>a.obraId===id)){
      actividades=actividades.filter(a=>a.obraId!==id);
