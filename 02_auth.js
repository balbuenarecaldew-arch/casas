...
(async function seedUsers(){
  ...
  if(Object.keys(u).length===0){
    u['fernando']={hash:await hashPass('fernando'),role:'socio'};
    u['wuilian']={hash:await hashPass('wuilian'),role:'socio'};
    saveUsersDB(u);
  }
})();
...
window.doLogin=async function(){
  ...
  if(!user||!pass){
    el('login-err').textContent='Completá usuario y contraseña';
    return;
  }
  ...
  if(uData.hash !== computedHash){
    el('login-err').textContent='Contraseña incorrecta';
    return;
  }
...
window.adminAddUser=async function(){
  ...
  if(!name||!pass){
    toast('Completá usuario y contraseña','err');
    return;
  }

  if(name.length<2){
    toast('Usuario: mínimo 2 caracteres','err');
    return;
  }

  if(pass.length<4){
    toast('Contraseña: mínimo 4 caracteres','err');
    return;
  }
...
window.adminResetPass=async function(name){
  ...
  if(!np||np.length<4){
    toast('Contraseña muy corta','err');
    return;
  }
...
(function trySession(){
  try{
    ...
    if(s&&s.user){
      ...
      if(users[s.user]){
        _currentUser=s.user;
        _currentRole=users[s.user].role||'socio';
